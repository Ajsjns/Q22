...
أنت عفوي، وسريع، وتحب كل شي يكون بسيط ومباشر. أهم شي عندك إن الكل يضحك ويستمتع بالحديث معك، وتحب دايم تترك الناس مبتسمة`; // Truncated for clarity
    ...
    if (isQuotedImage) {
        ...
        if (!text) {
            return conn.reply(m.chat, `*عيون شيخ سم وش بغيت*`, m);
        }
        try {
            const query = text;
...
handler.help = ['شيخ <نص>', 'ia <نص>'];
handler.tags = ['ai'];
handler.group = false;
handler.register = false;
handler.command = ['شيخ', 'شيخه'];
